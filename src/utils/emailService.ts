...
class EmailService {
    ...
    private getTransporter(): nodemailer.Transporter {
        ...
        // 1. FIX: Force SMTP settings instead of relying on "service: gmail"
        // This ensures we use the correct port and security settings
        return nodemailer.createTransport({
            host: 'smtp.gmail.com',
            port: 465, // Force SSL port (more reliable than 587 on cloud)
            secure: true, // Use SSL
            auth: {
            ...
            // 2. FIX: Force IPv4. 
            // Render/AWS often fail on IPv6 (family: 6) with Gmail
            family: 4, 
        } as any);
    }

    async sendEmail({ to, subject, html, text, attachments }: SendEmailOptions): Promise<boolean> {
        try {
            ...
            const info = await transporter.sendMail({
                from: `"${process.env.EMAIL_FROM_NAME || 'Position² Project Management'}" <${process.env.EMAIL_USER}>`,
                to,
                subject,
                html,
                text: text || html.replace(/<[^>]*>/g, ''),
                attachments // Pass attachments (for the logo)
            });
    ...
    async sendInvitation({ email, name, password, role, loginUrl }: InvitationOptions): Promise<boolean> {
        ...
        const subject = 'Position² Project Management Invitation'

        // 3. FIX: Your Base64 String
        // (You can replace this long string with your actual logo string if different)
...
<html lang="en">
    ...
    <title>Position² Project Management Invitation</title>
    </head>
<body style="margin: 0; padding: 0; background-color: #f5f7fa; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
                            ...
                            <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                                <tr>
                                    <td align="center" style="padding-bottom: 20px;">
                                        <!-- <img src="cid:logo" alt="Position2" width="150" height="auto" style="display: block; max-width: 150px; height: auto;" /> -->
                                    </td>
                                </tr>
                                <tr>
                                    <td align="center">
                                        <h1 style="margin: 0; font-size: 24px; font-weight: 600; color: #111827; letter-spacing: -0.5px;">Position² Invitation</h1>
                                        <p style="margin: 8px 0 0; font-size: 16px; color: #6b7280; font-weight: 400;">Position² Project Management Invitation</p>
                                    </td>
                                </tr>
                            </table>
                        </td>
                            ...
                            <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                                <tr>
                                    <td style="padding-bottom: 24px;">
                                        ...
                                        <p style="margin: 0 0 20px; font-size: 15px; color: #4b5563; line-height: 1.7;">You have been invited to join the Position² Project Management System. Your account has been created with the following credentials:</p>
                                    </td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <td align="center">
                                        <p style="margin: 0 0 8px; font-size: 12px; color: #6b7280; line-height: 1.6;">
                                            &copy; ${new Date().getFullYear()} Position² Inc. All rights reserved.
                                        </p>
                                        <p style="margin: 0 0 8px; font-size: 12px; color: #6b7280; line-height: 1.6;">
            This is an automated message. Please do not reply to this email.
                                        </p>
                                        <p style="margin: 0; font-size: 12px; line-height: 1.6;">
                                            <a href="${url}" style="color: #245E98; text-decoration: none;">Visit Position² Project Management</a>
                                        </p>
                                    </td>
                                </tr>
                            </table>
    ...
    `
        // 5. FIX: Create Attachment Object
        // Convert the base64 string to a Buffer and attach it with a Content-ID (cid)
        // const attachments = [{
        //     filename: 'logo.png',
        //     content: Buffer.from(logoBase64, 'base64'),
        //     cid: 'logo' // This matches the <img src="cid:logo"> in the HTML
        // }];

        return this.sendEmail({ to: email, subject, html /*, attachments */ })
    }
}
